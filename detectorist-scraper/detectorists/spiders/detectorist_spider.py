...
# This is a vBulletin forum, I wonder if all of them have similar XPath
# selection targets???

class DetectoristSpider(scrapy.Spider):
    name = 'detectorist'
    allowed_domains = ['gamesvillage.it']
    start_urls = ["http://www.gamesvillage.it/forum/showthread.php?1035875"]

    # Thread ID 12345 would look like:
    # 'http://example.com/showthread.php?t=12345...'
    ...
    def paginate(self, response, next_page_callback):
        ...
        # There are two identical ones with the same XPath, so just
        # extract_first.
    ...
    def parse_posts(self, response):
        ...
        # TODO: move this into parse_forum, b/c here the code runs every page
        # of the thread
        thread = ThreadItem()
        ...
        for post in response.xpath("//table[contains(@id,'post')]"):
            ...
            p['user_id'] = to_int(post.xpath(
                ".//a[@class='bigusername']/@href").re_first('u=(\d+)'))
            ...
            p['post_no'] = to_int(post.xpath(
                ".//tr/td/div[@class='normal'][1]/a//text()").extract_first())
